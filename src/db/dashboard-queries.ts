...
export async function getDailySpendByProvider(db: DB, days: number, project?: string) {
  ...
  const rows = await db
    ...
    .from(usageRecords)
    .where(sql`${usageRecords.periodStart} >= ${since}${projectFilter}`)
...
export async function getProviderBreakdown(db: DB, project?: string) {
  const now = new Date();
  const monthStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1)).toISOString();
  const projectFilter = project ? sql` AND ${usageRecords.project} = ${project}` : sql``;
...
export async function getModelTrends(db: DB, project?: string) {
  const now = new Date();
  ...
  const [thisWeek, lastWeek] = await Promise.all([
    db.select({
      ...
      .from(usageRecords)
      .where(sql`${usageRecords.periodStart} >= ${thisWeekStart}${projectFilter}`)
      .groupBy(usageRecords.provider, usageRecords.model),
    db.select({
      provider: usageRecords.provider,
      model: usageRecords.model,
      ...
      .from(usageRecords)
      .where(sql`${usageRecords.periodStart} >= ${lastWeekStart} AND ${usageRecords.periodStart} < ${thisWeekStart}${projectFilter}`)
      .groupBy(usageRecords.provider, usageRecords.model),
  ]);
